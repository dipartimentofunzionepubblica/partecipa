...
=begin		    
		    ...
		    def spidauth
				begin
					...
					@user = DecidimUser.from_omniauth(request.env["omniauth.auth"])
					if !@user.nil?
					...
					if @user
						...
						set_flash_message(:notice, :success, kind: "Spid") if is_navigational_format?
					else
						redirect_to new_decidim_user_registration_url
					end

				rescue StandardError => e
					puts "OmniauthCallbacksController.spidauth ==============>" + e.message
					puts "Backtrace:\n\t#{e.backtrace.join("\n\t")}"
