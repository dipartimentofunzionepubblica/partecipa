...
# Modificata per gestire i processi evidenziati in modo da non visualizzare quelli finiti
...
module Decidim
  module ParticipatoryProcesses
    ...
    class ParticipatoryProcessesController < Decidim::ParticipatoryProcesses::ApplicationController
	  ...
	  def promoted_participatory_processes
       # Qui otteniamo un Array di processi filtrati (solo quelli attivi)
       # Usiamo .query per assicurarci di operare sui risultati se necessario
       p_processes = published_processes.respond_to?(:query) ? published_processes.query : published_processes
       p_promoted = PromotedParticipatoryProcesses.new.query

       @promoted_participatory_processes ||= (p_processes | p_promoted).select(&:active?)
      end

      def promoted_collection
       # 1. Recuperiamo i gruppi (che sono oggetti Query, quindi serve .query)
       groups = promoted_participatory_process_groups.query

       # 2. Uniamo l'Array dei processi (già pronti) con l'Array dei gruppi
       # NON chiamare .query su promoted_participatory_processes perché è già un Array
       @promoted_collection ||= promoted_participatory_processes + groups.to_a
      end
