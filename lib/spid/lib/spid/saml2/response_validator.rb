...
module Spid
  module Saml2
	...
	class ResponseValidator # :nodoc:
	  ...
	  def response_id
		return true if !response.element_from_xpath("/samlp:Response/@ID").blank?
		@errors["response_id"] = 
	  ...
	  def version
		return true if response.element_from_xpath("/samlp:Response/@Version") == Spid::SAML_VERSION
		@errors["version"] = 
		  begin
	  ...
	  def issue_instant
		issue_instant = response.element_from_xpath("/samlp:Response/@IssueInstant")
		return true if !issue_instant.blank? && issue_instant == Time.parse(issue_instant).iso8601 && Time.parse(issue_instant).iso8601 >= Time.parse(authn_issue_instant).iso8601
		@errors["issue_instant"] = 
		  begin
			"Errore Spid 3: il parametro Response IssueInstant '#{issue_instant}' è un parametro necessario atteso come un date iso8601 successivo a AuthnRequest IssueInstant '#{authn_issue_instant}'"
		  end
		false
	  end
	  
	  def issuer_format
		format_element = response.document.elements["boolean(/samlp:Response/saml:Issuer[@Format='#{Spid::SAML_ISSUER_FORMAT}'])"]
		assertion_format_element = response.document.elements["boolean(/samlp:Response/saml:Assertion/saml:Issuer[@Format='#{Spid::SAML_ISSUER_FORMAT}'])"]
	  ...
	  def assertion_version
		return true if response.element_from_xpath("/samlp:Response/saml:Assertion/@Version") == Spid::SAML_VERSION
	  ...
	  def assertion_issue_instant
		issue_instant = response.element_from_xpath("/samlp:Response/saml:Assertion/@IssueInstant")
		return true if !issue_instant.blank? && !authn_issue_instant.blank? && issue_instant == Time.parse(issue_instant).iso8601.to_s && Time.parse(issue_instant).iso8601 >= Time.parse(authn_issue_instant).iso8601
		@errors["assertion_issue_instant"] = 
		  begin
			"Errore Spid 6: il parametro Assertion IssueInstant '#{issue_instant}' è atteso come un date iso8601 successivo a AuthnRequest IssueInstant '#{authn_issue_instant}'"
		  end
		false
	  end
	  
	  def assertion_id
		return true if !response.element_from_xpath("/samlp:Response/saml:Assertion/@ID").blank?
		@errors["assertion_id"] = 
	  ...
	  def assertion_nameid 
		return true if !response.element_from_xpath("/samlp:Response/saml:Assertion//saml:NameID/text()").blank?
	  ...
	  def assertion_nameid_format
		format = response.element_from_xpath("/samlp:Response/saml:Assertion//saml:NameID/@Format")
	  ...
	  def assertion_nameid_namequalifier
		namequalifier = response.element_from_xpath("/samlp:Response/saml:Assertion//saml:NameID/@NameQualifier")
	  ...
	  def assertion_subjectconfirmation
		return true if !response.document.elements["/samlp:Response/saml:Assertion//saml:SubjectConfirmation//saml:SubjectConfirmationData"].blank?
	  ...
	  def assertion_subjectconfirmation_method
		method = response.element_from_xpath("/samlp:Response/saml:Assertion//saml:SubjectConfirmation/@Method")
	  ...
	  def assertion_subjectconfirmationdata_recipient
		recipient = response.element_from_xpath("/samlp:Response/saml:Assertion//saml:SubjectConfirmation//saml:SubjectConfirmationData/@Recipient")
	  ...
	  def assertion_subjectconfirmationdata_inresponseto
		inresponseto = response.element_from_xpath("/samlp:Response/saml:Assertion//saml:SubjectConfirmation//saml:SubjectConfirmationData/@InResponseTo")
	  ...
	  def assertion_subjectconfirmationdata_notonorafter
		notonorafter = response.element_from_xpath("/samlp:Response/saml:Assertion//saml:SubjectConfirmation//saml:SubjectConfirmationData/@NotOnOrAfter")
		issue_instant = response.element_from_xpath("/samlp:Response/saml:Assertion/@IssueInstant")
		return true if !notonorafter.blank? && !issue_instant.blank? &&
			notonorafter == Time.parse(notonorafter).iso8601.to_s && 
				Time.parse(notonorafter).iso8601 > Time.parse(issue_instant).iso8601
		@errors["assertion_subjectconfirmationdata_notonorafter"] =
			begin
				"Errore Spid 15: il parametro Assertion SubjectConfirmationData NotOnOrAfter '#{notonorafter}' è un parametro necessario, atteso come un date iso8601 successivo a Assertion IssueInstant '#{issue_instant}'"
			end
		false
	  end
	  
	  def assertion_conditions
		conditions = response.document.elements["boolean(/samlp:Response/saml:Assertion//saml:Conditions/*)"]
	  ...
	  def assertion_conditions_notbefore
		notbefore = response.element_from_xpath("/samlp:Response/saml:Assertion//saml:Conditions/@NotBefore")
		return true if !notbefore.blank? && 
			notbefore == Time.parse(notbefore).iso8601.to_s 
	  ...
	  def assertion_authnstatement
		authnstatement = response.document.elements["boolean(/samlp:Response/saml:Assertion//saml:AuthnStatement/*)"]
		return true if authnstatement == true
	  ...
	  def assertion_authnstatement_authncontext
		authnstatement_authncontext = response.document.elements["boolean(/samlp:Response/saml:Assertion/saml:AuthnStatement/saml:AuthnContext/*)"]
		return true if authnstatement_authncontext == true
	  ...
	  def assertion_authnstatement_authncontext_authncontextclassref
		assertion_authnstatement_authncontext_authncontextclassref = response.element_from_xpath("/samlp:Response/saml:Assertion//saml:AuthnStatement/saml:AuthnContext/saml:AuthnContextClassRef/text()")
	  ...
	  def assertion_attributestatement
		assertion_attributestatement = response.document.elements["/samlp:Response/saml:Assertion//saml:AttributeStatement/*"]
